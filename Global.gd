...
func _ready():
	if get_tree().current_scene.name == "1c":
		pass
...
func update_score(s):
	...
	if get_tree().current_scene.name == "Game":
		...
		get_node_or_null("/root/Game/UI/HUD/Score").text = "Score: " + str(save_data["general"]["score"])

func update_health(h):
	save_data["general"]["health"] += h
	get_node_or_null("/root/Game/UI/HUD/Health").text = "Health: " + str(save_data["general"]["health"])
